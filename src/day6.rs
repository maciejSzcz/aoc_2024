...
fn get_visited_indexes_until_obstacle(
    ...
    loop {
        ...
        println!("{}, {}, {}", current_element, current_row, current_col);
        if current_element.to_string() == "#" {
            break;
        }

        elements.push((current_row, current_col));
        last_location = (
            current_row.try_into().unwrap(),
            current_col.try_into().unwrap(),
        );
...
fn get_move(board: &Vec<Vec<char>>) -> Result<usize, ()> {
